...
var disclaimer_description = $('<div>');
disclaimer_description.attr('id', 'disclaimer_description');
disclaimer_description.css({
  'width':      '100%',
  'max-height': 'auto'
});
disclaimer_description.load('disclaimer.html');
disclaimer.append(disclaimer_description);
...
$(window).on('load', function() {
  /**
   * 本来ならば、fitToBodySize()を単純にコールしたいが
   * レンダリングが完了せずbodyのサイズが確定していないことがあるため
   * テンポを遅らせるために、setTimeoutしている。
   */
  setTimeout(fitToBodySize);
});

$(window).on('resize', fitToBodySize);
